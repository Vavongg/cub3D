...
/*   Updated: 2025/12/10 15:32:29 by ainthana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	assign_color(char *str, t_color *color)
{
	...
	{
		free_split(rgb);
		return (error_msg("Error : non-numeric values"));
	}
	color->r = ft_atoi(rgb[0]);
	color->g = ft_atoi(rgb[1]);
	color->b = ft_atoi(rgb[2]);
...
{
	if (!tokens[1] || tokens[2])
		return (error_msg("Error : color config requires exactly one value"));
	if (!ft_strncmp(tokens[0], "F", 2))
	{
		if (config->floor.r != -1)
			return (error_msg("Error : floor color defined multiple times"));
		return (assign_color(tokens[1], &config->floor));
	}
	else if (!ft_strncmp(tokens[0], "C", 2))
	{
		if (config->ceiling.r != -1)
			return (error_msg("Error : ceiling color defined multiple times"));
		return (assign_color(tokens[1], &config->ceiling));
	}
	return (1);
}
